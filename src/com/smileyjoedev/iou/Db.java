...
public class Db {
	...
	public static void output_table_exists(SQLiteDatabase database, String tableName){
		if(Db.is_table_exists(database, tableName)){
			Debug.d("table " + tableName + " exists");
		} else {
			Debug.d("table " + tableName + " does not exist");
		}
	}
	...
	public static void output_column_exists(SQLiteDatabase database, String tableName, String colName){
		if(Db.is_column_exists(database, tableName, colName)){
			Debug.d("coloumn " + colName + " exists in table " + tableName);
		} else {
			Debug.d("coloumn " + colName + " does not exist in table " + tableName);
		}
	}
	...
	public static void output_table_details(SQLiteDatabase database, String tableName){
		...
		if(c.getCount() > 0){
			c.moveToFirst();
			do{
				Debug.d("table name", tableName);
				Debug.d();
				Debug.d("cid", c.getString(c.getColumnIndex("cid")));
				Debug.d("name", c.getString(c.getColumnIndex("name")));
				Debug.d("type", c.getString(c.getColumnIndex("type")));
				Debug.d("notnull", c.getString(c.getColumnIndex("notnull")));
				Debug.d("dflt_value", c.getString(c.getColumnIndex("dflt_value")));
				Debug.d("pk", c.getString(c.getColumnIndex("pk")));
				Debug.d();				
			}while(c.moveToNext());
		}
	}
